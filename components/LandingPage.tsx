    ...
    <div className="min-h-screen bg-white text-slate-900 selection:bg-indigo-100 scroll-smooth">
            ...
            {user ? (
              <div className="relative">
                <button 
                  ...
                  className="flex items-center gap-3 pl-2 pr-4 py-1.5 bg-slate-50 border border-slate-200 rounded-2xl hover:border-indigo-600 transition-all"
                >
                  <img src={user.avatar} className="w-8 h-8 rounded-xl" alt={user.name} />
                  <span className="text-sm font-black text-slate-900">{user.name.split(' ')[0]}</span>
                  <svg className={`w-4 h-4 text-slate-400 transition-transform ${showDropdown ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 9l-7 7-7-7" />
                  </svg>
                </button>
                
                {showDropdown && (
                  <div className="absolute top-full right-0 mt-2 w-56 bg-white rounded-2xl border border-slate-100 shadow-2xl py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                    <button 
                      onClick={onNavigateToDashboard}
                      className="w-full flex items-center gap-3 px-4 py-3 text-sm font-bold text-slate-600 hover:bg-slate-50 hover:text-indigo-600 transition-all"
                    >
                    ...
                    <button 
                      onClick={onLogout}
                      className="w-full flex items-center gap-3 px-4 py-3 text-sm font-bold text-rose-500 hover:bg-rose-50 transition-all"
                    >
                      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m-4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /></svg>
                      Sign Out
                    </button>
                  </div>
                )}
              ...
              <>
                <button 
                  onClick={onLogin}
                  className="px-6 py-2.5 rounded-xl border border-slate-200 text-sm font-black uppercase tracking-widest hover:bg-slate-50 transition-all"
                >
                  Sign In
                </button>
                <button 
                  onClick={onGetStarted}
                  className="px-6 py-2.5 bg-indigo-600 text-white rounded-xl text-sm font-black uppercase tracking-widest shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all"
                >
                  Get Started
                </button>
              </>
      ...
      <section className="pt-40 pb-20 px-6 relative overflow-hidden">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-7xl h-full pointer-events-none">
          <div className="absolute top-20 left-10 w-96 h-96 bg-indigo-50 rounded-full blur-[120px]"></div>
          <div className="absolute bottom-20 right-10 w-96 h-96 bg-purple-50 rounded-full blur-[120px]"></div>
        </div>

        <div className="max-w-5xl mx-auto text-center relative z-10">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-50 rounded-full mb-8">
            <span className="flex h-2 w-2 rounded-full bg-indigo-600 animate-pulse"></span>
            <span className="text-[10px] font-black uppercase tracking-widest text-indigo-600">Powered by Gemini 3.0</span>
          </div>
          <h1 className="text-5xl md:text-7xl lg:text-8xl font-black text-slate-900 tracking-tighter mb-8 leading-[0.9]">
          ...
          <p className="text-lg md:text-2xl text-slate-500 max-w-3xl mx-auto font-medium leading-relaxed mb-12">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <button 
              ...
              className="w-full sm:w-auto px-10 py-5 bg-slate-900 text-white rounded-2xl text-lg font-black uppercase tracking-widest shadow-2xl hover:bg-black transition-all transform hover:-translate-y-1"
            >
            ...
            {!user && (
              <button 
                onClick={onLogin}
                className="w-full sm:w-auto px-10 py-5 bg-white border border-slate-200 text-slate-900 rounded-2xl text-lg font-black uppercase tracking-widest hover:bg-slate-50 transition-all shadow-sm"
              >
                Login to Portal
              </button>
            )}
      ...
      <section id="features" className="py-24 bg-slate-50">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-20">
            <h2 className="text-4xl font-black text-slate-900 tracking-tight mb-4">A Holistic Approach to Fluency</h2>
            <p className="text-slate-500 font-medium">Four core pillars of learning integrated into one seamless platform.</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {features.map((f, i) => (
              <div 
                key={i} 
                className="group bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all cursor-default"
              >
                <div className={`w-14 h-14 bg-${f.color}-600 rounded-2xl flex items-center justify-center mb-8 shadow-lg shadow-${f.color}-100 text-white`}>
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="text-center mb-20">
            <h2 className="text-3xl md:text-5xl font-black text-slate-900 tracking-tight mb-4 leading-tight">
              Trusted by Thousands of <span className="text-indigo-600">Future Polyglots</span>
            </h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {stats.map((stat, i) => (
              <div 
                key={i} 
                className="group relative p-8 rounded-[2.5rem] bg-white border border-slate-100 shadow-[0_10px_40px_-15px_rgba(0,0,0,0.05)] hover:shadow-[0_25px_60px_-15px_rgba(79,70,229,0.15)] transition-all duration-500 hover:-translate-y-2 hover:border-indigo-100"
              >
                <div className="flex flex-col items-center text-center">
                  <div className="w-16 h-16 rounded-2xl bg-indigo-50 border border-indigo-100 flex items-center justify-center mb-6 group-hover:bg-indigo-600 group-hover:scale-110 transition-all duration-500">
                  ...
                  <p className="text-4xl md:text-5xl font-black text-slate-900 mb-2 tracking-tighter group-hover:text-indigo-600 transition-colors">
                    {stat.value}
                  </p>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] group-hover:text-indigo-400 transition-colors">
                    {stat.label}
                  </p>
                </div>
              </div>
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          <div className="pt-12 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8">
            ...
            <div className="flex items-center gap-2">
              <span className="text-[9px] font-black text-slate-800 uppercase tracking-[0.4em]">Propelled by</span>
